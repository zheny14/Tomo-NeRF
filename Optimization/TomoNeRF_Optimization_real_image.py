...
"In optimization,only 2D labels participate in optimization. 3D labels is only for calculating accuracy as a reference"
...
"training samples here  is the sample for optimization"
num_training_sample = 5
#num_testing_sample= 8
#image size
h = 100
w = 150

# batch size
BATCH_SIZE = 1024 #for number of probing X-rays per samples
IMAGE_BATCH_SIZE=12 #for number of samples

weight_factor = 25  # weight factor for MSE for 2D labels

"Load and order data path"
# Data path for training and testing datasets
DATA_PATH = "Real_images_for_optimization"
...
#optimizer = optim.SGD(model.parameters(), lr=0.01,momentum=0.9,weight_decay=0.0005) # tested
...
"load model"
model.load_state_dict(torch.load('Trained_model_before_optimization.pth'))
...
def train_meta(model, iterator_set,image_set,batch_order_list, optimizer, criterion_1, criterion_2, device):
    ...
    for i in range(len(batch_order_list)):
        ...
        "During optimization, only 2D labels are available"
        loss_2 = criterion_2(grayscale_pred, z)
        loss =weight_factor* loss_2
...
for epoch in range(EPOCHS):
    ...
    torch.save(model.state_dict(),'Optimization_real_six_view_EPOCH_' + str(epoch) + '.pth')
    #torch.save(model.state_dict(),'F:\\particle_data\\Trained_model\\train_cnn\\data_output_test\\fine_tune_test_large_sphere_cnn_model_EPOCH_' + str(epoch) + '.pth')




"##########################################################################"
df = pd.DataFrame({"train loss" : train_losses, "train acc": train_accs})
